<template>
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    <div
    v-if="loader"
    class="fixed w-100 h-100 opacity-80 bg-blue-800 inset-0 z-50 flex items-center justify-center">
    ...
    <div class="container">
      ...
      <add-ticker :tickWasAdd="tickWasAdd" @add-ticker="add" />
      ...
      <template v-if="tickers.length">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div v-for="t of filteredList" :key="t.name" @click="select(t)" :class="{ 'border-4': sel === t }"
...
export default {
  ...
  methods: {
    ...
    add(ticker) {
      ...
      this.tickWasAdd = false
      }
      else {
        this.tickWasAdd = true
      }
    },
    removeTicker(t) {
    ...
    pageChange() {
      if(this.filter) {
        return this.page<this.filteredList.length/6
      }
      return this.page<this.tickers.length/6
    }
  },
  ...
  computed: {
    ...
    normalizeGraph() {
      ...
      return this.graph.map(price => 5 + (price - this.minValue) * 95 / (this.maxValue - this.minValue))
    },
    startPage () {
  ...
  watch: {
    tickers: {
      handler() {localStorage.setItem("crypto-list", JSON.stringify(this.tickers))},
      deep:true
    },
    filter() {
      this.page = 1
